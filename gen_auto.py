...
for ex in ex_order:
    ...
    for x in ex_items:
        ...
        if p >= 1000 or p == int(p):
            ...
            ps = f'{p:,.2f}'
        ois = f'{oi/10000:.1f}�? if oi>=10000 else str(oi)
        rp = int(rate*100)
        ms = f'{ml/10000:.1f}万元' if ml>=10000 else f'{ml:.0f}�?
        sym = x["contract"]
        html += '<div class="cd" style="border-color:'+ex_colors[ex]+'" data-code="'+sym+'" data-mult="'+str(m)+'" data-rate="'+str(rate)+'"><div class="ct"><span class="cn">'+name+'</span><span class="ck">'+code+'</span><span class="ck" style="font-size:11px;color:#64748b">['+sym+']</span></div><div class="cb"><div class="cp" id="p_'+sym+'">'+ps+'</div><div class="cr"><div class="ci"><span>持仓</span><strong class="ib" id="oi_'+sym+'">'+ois+'</strong></div><div class="ci"><span>保证�?/span><strong class="ib" style="color:'+ex_colors[ex]+'" id="mg_'+sym+'">'+str(rp)+'%</strong></div><div class="ci"><span>每手</span><strong class="im" id="ml_'+sym+'">'+ms+'</strong></div></div></div></div>'
    html += '</div></div>'
html += '</div><div class="ft">实时行情来源:新浪财经 �?0秒自动更�?价格自动刷新，保证金率同步计�?/div>'
html += '<script>'
...
html += 'function fo(v){return v>=10000?(v/10000).toFixed(1)+"�?:String(v);}'
html += 'function fm(v){return v>=10000?(v/10000).toFixed(1)+"万元":Math.round(v).toLocaleString()+"�?;}'
html += 'var _psnap={};function deepCheck(){var n=document.getElementById("selfCheckStatus");var e=0,s=0,k=0;for(var i=0;i<SC.length;i++){var sm=SC[i];var p=document.getElementById("p_"+sm);if(!p)continue;var v=parseFloat(p.textContent.replace(/[^0-9.\-]/g,""));if(isNaN(v)||v<=0)e++;else{k++;if(_psnap[sm]!==undefined&&_psnap[sm]===v)s++}_psnap[sm]=v}var m="";if(e)m+=e+"异常 ";if(s)m+=s+"未更�?";if(!m)m="正常";m+=" | "+k+"/"+SC.length;if(n){n.textContent=m;n.style.color=e>0?"#ef4444":s>0?"#eab308":"#22c55e"}if(e||s)console.warn("自检:"+e+"异常",s+"未更�?)}function selfCheck(){var e=[],k=0;var n=document.getElementById("selfCheckStatus");for(var i=0;i<SC.length;i++){var sm=SC[i];var pe=document.getElementById("p_"+sm);if(pe){var v=parseFloat(pe.textContent.replace(/[^0-9.\-]/g,""));if(isNaN(v)||v<=0)e.push(sm);else k++}}var msg=k+"/"+SC.length+" \u6b63\u5e38";if(e.length)msg+="; "+e.length+" \u5f02\u5e38";if(n)n.textContent=msg;if(e.length)console.warn("\u81ea\u68c0: "+e.length+" \u4e2a\u5408\u7ea6\u5f02\u5e38: "+e.join(","));return{ok:k,err:e.length}}function fq(){SI.textContent="更新�?..";'
