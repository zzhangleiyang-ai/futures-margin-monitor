...
html += 'function selfCheck(){var e=[],k=0;var n=document.getElementById("selfCheckStatus");for(var i=0;i<SC.length;i++){var sm=SC[i];var pe=document.getElementById("p_"+sm);if(pe){var v=parseFloat(pe.textContent.replace(/[^0-9.\-]/g,""));if(isNaN(v)||v<=0)e.push(sm);else k++}}var msg=k+"/"+SC.length+" \u6b63\u5e38";if(e.length)msg+="; "+e.length+" \u5f02\u5e38";if(n)n.textContent=msg;if(e.length)console.warn("\u81ea\u68c0: "+e.length+" \u4e2a\u5408\u7ea6\u5f02\u5e38: "+e.join(","));return{ok:k,err:e.length}}function fq(){SI.textContent="更新中...";'
