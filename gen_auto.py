...
html += '<div class="w sr"><input class="sri" id="searchBox" type="text" placeholder="搜索品种名称、代码..." oninput="filterCards()"/><span class="rc" id="countDisplay">'+str(len(items))+'个</span></div>'
...
html += '}'
html += 'function filterCards(){var q=document.getElementById("searchBox").value.trim().toLowerCase();var cards=document.querySelectorAll(".cd");var show=0;var secs=document.querySelectorAll(".se");for(var i=0;i<cards.length;i++){var c=cards[i];var t=c.textContent.toLowerCase();if(!q||t.indexOf(q)>-1){c.style.display="";show++}else{c.style.display="none"}}for(var j=0;j<secs.length;j++){var s=secs[j];var vis=s.querySelectorAll(".cd[style*=\"display: none\"]");var al=s.querySelectorAll(".cd").length;if(al>0&&vis.length===al){s.style.display="none"}else{s.style.display=""}}document.getElementById("countDisplay").textContent=(q?show+"/":"")+SC.length+"个"}fq();'
